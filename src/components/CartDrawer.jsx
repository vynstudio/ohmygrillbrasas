    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: 20, fontWeight: 600, color: '#1A0800', margin: 0 }}>
              Tu pedido
            </h2>
            {itemCount > 0 && (
              <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 12, color: '#D4956A', margin: '2px 0 0' }}>
                {itemCount} {itemCount === 1 ? 'artículo' : 'artículos'}
              </p>
            )}
          </div>
          <button onClick={onClose} style={{
            background: '#2C1200', border: 'none', borderRadius: '50%',
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 24px' }}>
          ...
          {itemCount === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px 20px', color: '#D4956A' }}>
              ...
              <p style={{ fontFamily: "'Fraunces', serif", fontSize: 18, fontWeight: 600, color: '#1A0800', marginBottom: 8 }}>
            ...
            <>
                ...
                {items.map(item => (
                  <div key={item.id} style={{
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 13, fontWeight: 500, color: '#1A0800', margin: 0 }}>
                        {item.name}
                      </p>
                      <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 12, color: '#D4956A', margin: '1px 0 0' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                      <button onClick={() => updateQty(item.id, item.qty - 1)} style={{
                        ...
                        fontSize: 14, color: '#D4956A', fontWeight: 500,
                      }}>−</button>
                      <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: 13, fontWeight: 500, color: '#1A0800', minWidth: 16, textAlign: 'center' }}>
                        {item.qty}
                      </span>
                      <button onClick={() => updateQty(item.id, item.qty + 1)} style={{
                        width: 26, height: 26, border: 'none', background: '#1A0800',
                        borderRadius: '50%', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
                        fontSize: 14, color: '#fff', fontWeight: 500,
                      }}>+</button>
                    </div>
                    <span style={{ fontFamily: "'Fraunces', serif", fontSize: 15, fontWeight: 600, color: '#1A0800', minWidth: 48, textAlign: 'right' }}>
              ...
              <div style={{ marginBottom: 16 }}>
                <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 12, fontWeight: 500, color: '#1A0800', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.8px' }}>
                  Tipo de entrega
                </p>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  {[{ id: 'delivery', label: '🛵 A domicilio' }, { id: 'pickup', label: '🏪 Recogida' }].map(opt => (
                    <button key={opt.id} onClick={() => setDeliveryType(opt.id)} style={{
                      ...
                      color: deliveryType === opt.id ? '#ffd43a' : '#D4956A',
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  <select
                    ...
                    style={{
                      ...
                      fontSize: 13, color: '#1A0800', cursor: 'pointer',
                    }}
                  >
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <textarea
                  ...
                  style={{
                    width: '100%', padding: '10px 12px', border: '1px solid #D4CFC9',
                    borderRadius: 10, background: '#fff', fontFamily: "'Outfit', sans-serif",
                    fontSize: 13, color: '#1A0800', resize: 'vertical', boxSizing: 'border-box',
                  }}
                />
              </div>
            </>
        ...
        {itemCount > 0 && (
          <div style={{ padding: '16px 24px', borderTop: '1px solid #2A1A00', background: '#fff' }}>
            <div style={{ marginBottom: 12 }}>
              {[
                ...
                <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                  <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: 13, color: '#D4956A' }}>{row.label}</span>
                  <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: 13, color: '#1A0800', fontWeight: 500 }}>{row.value}</span>
                </div>
              ))}
              <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 10, borderTop: '1px solid #2A1A00', marginTop: 6 }}>
                <span style={{ fontFamily: "'Fraunces', serif", fontSize: 17, fontWeight: 600, color: '#1A0800' }}>Total</span>
            ...
            <button
              ...
              style={{
                width: '100%', padding: '14px', background: '#1A0800', color: '#fff',
