...
export default function Navbar({ activePage, onNavigate, onCartOpen }) {
  ...
  return (
    <>
      <div style={{ background:'#E85820', color:'#fff', textAlign:'center', fontSize: isMobile ? 11 : 12, letterSpacing:'1px', padding: isMobile ? '7px 12px' : '8px 16px', fontFamily:"'Outfit',sans-serif", fontWeight:500, lineHeight:1.4 }}>
        ...
        <div style={{ maxWidth:1200, margin:'0 auto', padding:'0 20px', display:'flex', alignItems:'center', justifyContent:'space-between', height:60 }}>
          ...
          {!isMobile && (
            <div style={{ display:'flex', gap:28, alignItems:'center' }}>
              {links.map(l => (
                <button key={l.id} onClick={() => handleNav(l.id)} style={{ background:'none', border:'none', cursor:'pointer', fontSize:13, fontFamily:"'Outfit',sans-serif", fontWeight: activePage===l.id ? 500 : 400, color: activePage===l.id ? '#E85820' : '#7A6E63', padding:'4px 0', borderBottom: activePage===l.id ? '2px solid #E85820' : '2px solid transparent', transition:'all 0.15s' }}>
                  {l.label}
                </button>
              ))}
            </div>
        ...
        {isMobile && (
          <div style={{ maxHeight: mobileOpen ? 400 : 0, overflow:'hidden', transition:'max-height 0.35s cubic-bezier(0.4,0,0.2,1)', borderTop: mobileOpen ? '1px solid #EDE9E3' : 'none', background:'#fff' }}>
            <div style={{ padding:'8px 0 16px' }}>
              {links.map(l => (
                <button key={l.id} onClick={() => handleNav(l.id)} style={{ display:'block', width:'100%', textAlign:'left', background:'none', border:'none', cursor:'pointer', padding:'13px 24px', fontFamily:"'Outfit',sans-serif", fontSize:16, fontWeight: activePage===l.id ? 500 : 400, color: activePage===l.id ? '#E85820' : '#1C1A14', borderLeft: activePage===l.id ? '3px solid #E85820' : '3px solid transparent' }}>
                  {l.label}
                </button>
              ))}
              <div style={{ padding:'8px 24px 0' }}>
                <button onClick={() => handleNav('menu')} style={{ width:'100%', background:'#E85820', color:'#fff', border:'none', borderRadius:12, padding:'14px', fontFamily:"'Outfit',sans-serif", fontSize:15, fontWeight:500, cursor:'pointer' }}>
                  Pedir ahora →
                </button>
              </div>
            </div>
