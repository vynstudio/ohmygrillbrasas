...
function AdminLogin({ onLogin }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0C0A06', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ background: '#1A0800', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 24, padding: '48px 40px', width: '100%', maxWidth: 380, textAlign: 'center' }}>
...
function OrderCard({ order, onStatusChange }) {
  ...
  return (
    <div style={{ background: '#fff', border: `1px solid ${order.status === 'pending' ? '#EF9F27' : '#3D1800'}`, borderRadius: 16, overflow: 'hidden', transition: 'box-shadow 0.2s', boxShadow: order.status === 'pending' ? '0 0 0 3px rgba(239,159,39,0.15)' : 'none' }}>
      {/* Header */}
      <div style={{ padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', borderBottom: expanded ? '1px solid #F0EDE8' : 'none' }} onClick={() => setExpanded(!expanded)}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            <span style={{ fontFamily: "'Fraunces', serif", fontSize: 15, fontWeight: 600, color: '#1A0800' }}>{order.id}</span>
          ...
          <p style={{ fontSize: 13, color: '#D4956A', margin: '3px 0 0' }}>{order.customer} · {timeAgo(order.createdAt)}</p>
        </div>
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          <p style={{ fontFamily: "'Fraunces', serif", fontSize: 18, fontWeight: 600, color: '#ffd43a', margin: 0 }}>€{order.total.toFixed(2)}</p>
          <p style={{ fontSize: 11, color: '#D4956A', margin: '2px 0 0' }}>{order.deliveryType === 'pickup' ? '🏪 Recogida' : '🛵 Entrega'}</p>
        </div>
        <span style={{ fontSize: 16, color: '#D4956A', transform: expanded ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}>▾</span>
      </div>
      ...
      {expanded && (
        <div style={{ padding: '16px 18px' }}>
          ...
          <div style={{ marginBottom: 14 }}>
            <p style={{ fontSize: 11, letterSpacing: '1.5px', color: '#D4956A', fontWeight: 600, textTransform: 'uppercase', margin: '0 0 8px' }}>Artículos</p>
            {order.items.map((item, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, padding: '5px 0', borderBottom: '1px solid #1A1000' }}>
                <span style={{ color: '#1A0800' }}>{item.qty}× {item.name}</span>
                <span style={{ color: '#D4956A', fontWeight: 500 }}>€{(item.price * item.qty).toFixed(2)}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
            {[
              ...
              <div key={item.label}>
                <p style={{ fontSize: 11, color: '#D4956A', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '1px', margin: '0 0 2px' }}>{item.label}</p>
                <p style={{ fontSize: 13, color: '#1A0800', margin: 0 }}>{item.val}</p>
              </div>
            ))}
          </div>

          {order.address && (
            <div style={{ background: '#1A0800', borderRadius: 10, padding: '10px 12px', marginBottom: 12 }}>
              <p style={{ fontSize: 11, color: '#D4956A', margin: '0 0 2px' }}>📍 Dirección</p>
              <p style={{ fontSize: 13, color: '#1A0800', margin: 0 }}>{order.address}</p>
            </div>
          )}
          ...
          {order.notes && (
            <div style={{ background: '#FEF3EE', border: '1px solid rgba(232,88,32,0.2)', borderRadius: 10, padding: '10px 12px', marginBottom: 14 }}>
              ...
              <p style={{ fontSize: 13, color: '#D4956A', margin: 0 }}>{order.notes}</p>
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {cfg.next && (
              <button onClick={() => onStatusChange(order.id, cfg.next)} style={{ flex: 1, background: '#1A0800', color: '#fff', border: 'none', borderRadius: 10, padding: '11px', fontFamily: "'Outfit', sans-serif", fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>
                {cfg.nextLabel} →
              </button>
            )}
...
function MenuEditor() {
  ...
  if (loading) return (
    <div style={{ textAlign: 'center', padding: '48px 20px', color: '#D4956A' }}>
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          <h3 style={{ fontFamily: "'Fraunces', serif", fontSize: 20, fontWeight: 600, color: '#1A0800', margin: 0 }}>Editor de carta</h3>
          <p style={{ fontSize: 13, color: '#D4956A', margin: '3px 0 0' }}>Activa/desactiva platos y actualiza precios</p>
        </div>
<div style={{ display:'flex', alignItems:'center', gap:10 }}>
      ...
      {Object.entries(grouped).map(([cat, items]) => (
        <div key={cat} style={{ marginBottom: 28 }}>
          <h4 style={{ fontFamily: "'Fraunces', serif", fontSize: 16, fontWeight: 600, color: '#1A0800', margin: '0 0 10px', display: 'flex', alignItems: 'center', gap: 10 }}>
            {catLabels[cat]}
            <span style={{ width: 32, height: 1, background: '#3D1800', display: 'inline-block' }} />
          </h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {items.map(item => (
              <div key={item.id} style={{ background: item.available ? '#fff' : '#2C1200', border: '1px solid #2A1A00', borderRadius: 12, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12, opacity: item.available ? 1 : 0.6 }}>
                <span style={{ fontSize: 22 }}>{item.emoji}</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: 14, fontWeight: 500, color: '#1A0800', margin: 0 }}>{item.name}</p>
                  <p style={{ fontSize: 11, color: '#D4956A', margin: '1px 0 0' }}>{item.weight}</p>
                </div>
                {/* Price editor */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  <span style={{ fontSize: 13, color: '#D4956A' }}>€</span>
                  <input
                    type="number" step="0.5" min="0"
                    value={item.price}
                    onChange={e => handlePriceChange(item.id, e.target.value)}
                    style={{ width: 64, padding: '6px 8px', border: '1.5px solid #2A1A00', borderRadius: 8, fontFamily: "'Outfit', sans-serif", fontSize: 14, fontWeight: 500, color: '#1A0800', textAlign: 'center', outline: 'none' }}
                    onFocus={e => e.target.style.borderColor = '#ffd43a'}
                    onBlur={e => e.target.style.borderColor = '#3D1800'}
                  />
                </div>
                {/* Toggle */}
                <button onClick={() => handleToggle(item.id)} style={{ display: 'flex', alignItems: 'center', gap: 6, background: item.available ? '#EDFBF3' : '#2C1200', border: 'none', borderRadius: 20, padding: '6px 12px', cursor: 'pointer', fontFamily: "'Outfit', sans-serif", fontSize: 12, fontWeight: 500, color: item.available ? '#1a7a4a' : '#D4956A', whiteSpace: 'nowrap' }}>
                  <span style={{ width: 8, height: 8, borderRadius: '50%', background: item.available ? '#1a7a4a' : '#D4956A', display: 'inline-block' }} />
      ...
      {stats.map(s => (
        <div key={s.label} style={{ background: '#fff', border: '1px solid #2A1A00', borderRadius: 14, padding: '16px 18px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            ...
            <span style={{ fontSize: 11, color: '#D4956A', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '1px' }}>{s.label}</span>
          </div>
          <p style={{ fontFamily: "'Fraunces', serif", fontSize: 28, fontWeight: 600, color: s.color, margin: '0 0 3px', lineHeight: 1 }}>{s.value}</p>
          <p style={{ fontSize: 12, color: '#D4956A', margin: 0 }}>{s.sub}</p>
        </div>
      ))}
    </div>
  );
...
export default function AdminPage() {
  ...
  return (
    <div style={{ fontFamily: "'Outfit', sans-serif", background: '#2C1200', minHeight: '100vh' }}>

      {/* Top bar */}
      <div style={{ background: '#1A0800', padding: '0 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 56, position: 'sticky', top: 0, zIndex: 50 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px 20px' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 16, overflowX: 'auto', paddingBottom: 4 }}>
              {[{ id: 'all', label: 'Todos' }, ...Object.entries(STATUS_CONFIG).map(([id, cfg]) => ({ id, label: cfg.label }))].map(f => (
                <button key={f.id} onClick={() => setFilterStatus(f.id)} style={{ background: filterStatus === f.id ? '#1A0800' : '#fff', color: filterStatus === f.id ? '#fff' : '#D4956A', border: '1px solid #2A1A00', borderRadius: 20, padding: '7px 16px', fontFamily: "'Outfit', sans-serif", fontSize: 13, cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}>
            ...
            {filteredOrders.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '60px 20px', color: '#D4956A' }}>
                ...
                <p style={{ fontFamily: "'Fraunces', serif", fontSize: 18, color: '#1A0800', marginBottom: 6 }}>Sin pedidos</p>
        ...
        {tab === 'stats' && (
          <div>
            <h3 style={{ fontFamily: "'Fraunces', serif", fontSize: 22, fontWeight: 600, color: '#1A0800', margin: '0 0 20px' }}>Resumen de actividad</h3>

            {orders.length === 0 && (
              <div style={{ textAlign: 'center', padding: '48px', color: '#D4956A' }}>
                <div style={{ fontSize: 40, marginBottom: 12 }}>📊</div>
                <p style={{ fontFamily: "'Fraunces', serif", fontSize: 18, color: '#1A0800', marginBottom: 6 }}>Sin datos todavía</p>
            ...
            <div style={{ background: '#fff', border: '1px solid #2A1A00', borderRadius: 20, padding: '24px', marginBottom: 16 }}>
              <h4 style={{ fontFamily: "'Fraunces', serif", fontSize: 17, fontWeight: 600, color: '#1A0800', margin: '0 0 16px' }}>Ingresos por zona</h4>
              {Object.entries(
                orders.reduce((acc, o) => {
                ...
                return (
                  <div key={zone} style={{ marginBottom: 12 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                      <span style={{ fontSize: 13, color: '#1A0800' }}>{zone}</span>
                      <span style={{ fontSize: 13, fontWeight: 500, color: '#ffd43a' }}>€{rev.toFixed(0)}</span>
                    </div>
                    <div style={{ height: 6, background: '#2C1200', borderRadius: 3, overflow: 'hidden' }}>
            ...
            <div style={{ background: '#fff', border: '1px solid #2A1A00', borderRadius: 20, padding: '24px' }}>
              <h4 style={{ fontFamily: "'Fraunces', serif", fontSize: 17, fontWeight: 600, color: '#1A0800', margin: '0 0 16px' }}>Platos más pedidos</h4>
              {Object.entries(
                ...
                <div key={name} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '9px 0', borderBottom: i < 5 ? '1px solid #1A1000' : 'none' }}>
                  <span style={{ fontFamily: "'Fraunces', serif", fontSize: 16, fontWeight: 600, color: '#D4956A', minWidth: 20 }}>{i + 1}</span>
                  <span style={{ fontSize: 13, color: '#1A0800', flex: 1 }}>{name}</span>
