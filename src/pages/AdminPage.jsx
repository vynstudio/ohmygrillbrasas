  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0C0A06', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ background: '#1C1A14', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 24, padding: '48px 40px', width: '100%', maxWidth: 380, textAlign: 'center' }}>
        ...
        <button onClick={handle} style={{ width: '100%', background: '#F5C518', color: '#fff', border: 'none', borderRadius: 12, padding: '13px', fontFamily: "'Outfit', sans-serif", fontSize: 15, fontWeight: 500, cursor: 'pointer' }}>
          Entrar →
        </button>
      ...
      <div style={{ padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', borderBottom: expanded ? '1px solid #F0EDE8' : 'none' }} onClick={() => setExpanded(!expanded)}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            ...
            {order.status === 'pending' && <span style={{ fontSize: 10, background: '#F5C518', color: '#fff', padding: '2px 6px', borderRadius: 10, fontWeight: 700 }}>NUEVO</span>}
          </div>
          <p style={{ fontSize: 13, color: '#7A6E63', margin: '3px 0 0' }}>{order.customer} · {timeAgo(order.createdAt)}</p>
        </div>
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          <p style={{ fontFamily: "'Fraunces', serif", fontSize: 18, fontWeight: 600, color: '#F5C518', margin: 0 }}>€{order.total.toFixed(2)}</p>
        ...
        <div style={{ padding: '16px 18px' }}>
          ...
          {order.notes && (
            <div style={{ background: '#FEF3EE', border: '1px solid rgba(232,88,32,0.2)', borderRadius: 10, padding: '10px 12px', marginBottom: 14 }}>
              <p style={{ fontSize: 11, color: '#F5C518', margin: '0 0 2px', fontWeight: 600 }}>📝 Notas</p>
...
function MenuEditor() {
  ...
  if (loading) return (
    <div style={{ textAlign: 'center', padding: '48px 20px', color: '#9A8F85' }}>
      <div style={{ width: 32, height: 32, border: '3px solid #EDE9E3', borderTopColor: '#F5C518', borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto 12px' }} />
...
<div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          <button onClick={handleSave} disabled={saving} style={{ background: saved ? '#1a7a4a' : '#F5C518', color: '#fff', border: 'none', borderRadius: 10, padding: '10px 20px', fontFamily: "'Outfit', sans-serif", fontSize: 13, fontWeight: 500, cursor: saving ? 'not-allowed' : 'pointer', transition: 'background 0.2s', display:'flex', alignItems:'center', gap:6 }}>
...
function StatsSection({ orders }) {
  ...
  const stats = [
    ...
    { icon: '🧾', label: 'Ticket medio', value: `€${avgOrder.toFixed(0)}`, sub: 'Por pedido', color: '#F5C518' },
    ...
    <div style={{ fontFamily: "'Outfit', sans-serif", background: '#F5F1EC', minHeight: '100vh' }}>
      ...
      <div style={{ background: '#1C1A14', padding: '0 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 56, position: 'sticky', top: 0, zIndex: 50 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {pendingCount > 0 && (
            <span style={{ background: '#F5C518', color: '#fff', fontSize: 11, fontWeight: 700, padding: '3px 8px', borderRadius: 20 }}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #EDE9E3', padding: '0 20px', display: 'flex', gap: 4 }}>
        {tabs.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '14px 16px', fontFamily: "'Outfit', sans-serif", fontSize: 14, fontWeight: tab === t.id ? 500 : 400, color: tab === t.id ? '#F5C518' : '#7A6E63', borderBottom: `2px solid ${tab === t.id ? '#F5C518' : 'transparent'}`, display: 'flex', alignItems: 'center', gap: 6, whiteSpace: 'nowrap' }}>
            {t.label}
            {t.badge && <span style={{ background: '#F5C518', color: '#fff', fontSize: 10, fontWeight: 700, width: 18, height: 18, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{t.badge}</span>}
          </button>
        ))}
      </div>
                ...
                return (
                  <div key={zone} style={{ marginBottom: 12 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                      ...
                      <span style={{ fontSize: 13, fontWeight: 500, color: '#F5C518' }}>€{rev.toFixed(0)}</span>
                    </div>
                    <div style={{ height: 6, background: '#F5F1EC', borderRadius: 3, overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: `${pct}%`, background: '#F5C518', borderRadius: 3, transition: 'width 0.5s ease' }} />
                    </div>
                  </div>
                );
              })}
            ...
            <div style={{ background: '#fff', border: '1px solid #EDE9E3', borderRadius: 20, padding: '24px' }}>
              ...
              {Object.entries(
                ...
                <div key={name} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '9px 0', borderBottom: i < 5 ? '1px solid #F5F1EC' : 'none' }}>
                  ...
                  <span style={{ background: '#FEF3EE', color: '#F5C518', fontSize: 12, fontWeight: 600, padding: '3px 10px', borderRadius: 20 }}>{qty} uds</span>
                </div>
              ))}
            </div>
            </>}
