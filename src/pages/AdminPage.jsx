  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0C0A06', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ background: '#1A1500', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 24, padding: '48px 40px', width: '100%', maxWidth: 380, textAlign: 'center' }}>
        ...
        <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: 28, fontWeight: 600, color: '#FFF8DC', margin: '0 0 6px' }}>Panel de administración</h1>
        <p style={{ fontSize: 13, color: 'rgba(240,235,227,0.4)', margin: '0 0 32px' }}>OhMyGrill Brasas · Zaragoza</p>
        <input
          type="password" placeholder="Contraseña"
          value={pw} onChange={e => setPw(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && handle()}
          style={{ width: '100%', padding: '13px 16px', background: 'rgba(255,255,255,0.06)', border: `1.5px solid ${error ? '#E24B4A' : 'rgba(255,255,255,0.1)'}`, borderRadius: 12, color: '#FFF8DC', fontFamily: "'Outfit', sans-serif", fontSize: 15, marginBottom: 12, boxSizing: 'border-box', outline: 'none', textAlign: 'center', letterSpacing: '4px' }}
        />
...
function OrderCard({ order, onStatusChange }) {
  ...
  return (
    <div style={{ background: '#fff', border: `1px solid ${order.status === 'pending' ? '#EF9F27' : '#F0E4C8'}`, borderRadius: 16, overflow: 'hidden', transition: 'box-shadow 0.2s', boxShadow: order.status === 'pending' ? '0 0 0 3px rgba(239,159,39,0.15)' : 'none' }}>
      {/* Header */}
      <div style={{ padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', borderBottom: expanded ? '1px solid #F0EDE8' : 'none' }} onClick={() => setExpanded(!expanded)}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <p style={{ fontSize: 13, color: '#7A6030', margin: '3px 0 0' }}>{order.customer} · {timeAgo(order.createdAt)}</p>
        </div>
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          <p style={{ fontFamily: "'Fraunces', serif", fontSize: 18, fontWeight: 600, color: '#ffd43a', margin: 0 }}>€{order.total.toFixed(2)}</p>
          <p style={{ fontSize: 11, color: '#8B7355', margin: '2px 0 0' }}>{order.deliveryType === 'pickup' ? '🏪 Recogida' : '🛵 Entrega'}</p>
        </div>
      ...
      {expanded && (
        <div style={{ padding: '16px 18px' }}>
          ...
          <div style={{ marginBottom: 14 }}>
            <p style={{ fontSize: 11, letterSpacing: '1.5px', color: '#8B7355', fontWeight: 600, textTransform: 'uppercase', margin: '0 0 8px' }}>Artículos</p>
            {order.items.map((item, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, padding: '5px 0', borderBottom: '1px solid #FFF8E7' }}>
                <span style={{ color: '#1A1500' }}>{item.qty}× {item.name}</span>
                <span style={{ color: '#7A6030', fontWeight: 500 }}>€{(item.price * item.qty).toFixed(2)}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
            {[
              ...
              <div key={item.label}>
                <p style={{ fontSize: 11, color: '#8B7355', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '1px', margin: '0 0 2px' }}>{item.label}</p>
          ...
          {order.address && (
            <div style={{ background: '#FFFBEE', borderRadius: 10, padding: '10px 12px', marginBottom: 12 }}>
              <p style={{ fontSize: 11, color: '#8B7355', margin: '0 0 2px' }}>📍 Dirección</p>
          ...
          {order.notes && (
            <div style={{ background: '#FEF3EE', border: '1px solid rgba(232,88,32,0.2)', borderRadius: 10, padding: '10px 12px', marginBottom: 14 }}>
              ...
              <p style={{ fontSize: 13, color: '#7A6030', margin: 0 }}>{order.notes}</p>
            </div>
          )}
...
function MenuEditor() {
  ...
  if (loading) return (
    <div style={{ textAlign: 'center', padding: '48px 20px', color: '#8B7355' }}>
      <div style={{ width: 32, height: 32, border: '3px solid #F0E4C8', borderTopColor: '#ffd43a', borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto 12px' }} />
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: '#8B7355', margin: '3px 0 0' }}>Activa/desactiva platos y actualiza precios</p>
        </div>
      ...
      {Object.entries(grouped).map(([cat, items]) => (
        <div key={cat} style={{ marginBottom: 28 }}>
          <h4 style={{ fontFamily: "'Fraunces', serif", fontSize: 16, fontWeight: 600, color: '#1A1500', margin: '0 0 10px', display: 'flex', alignItems: 'center', gap: 10 }}>
            {catLabels[cat]}
            <span style={{ width: 32, height: 1, background: '#F0E4C8', display: 'inline-block' }} />
          </h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {items.map(item => (
              <div key={item.id} style={{ background: item.available ? '#fff' : '#FFF8E7', border: '1px solid #F0E4C8', borderRadius: 12, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12, opacity: item.available ? 1 : 0.6 }}>
                <span style={{ fontSize: 22 }}>{item.emoji}</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: 14, fontWeight: 500, color: '#1A1500', margin: 0 }}>{item.name}</p>
                  <p style={{ fontSize: 11, color: '#8B7355', margin: '1px 0 0' }}>{item.weight}</p>
                </div>
                {/* Price editor */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  ...
                  <input
                    ...
                    style={{ width: 64, padding: '6px 8px', border: '1.5px solid #F0E4C8', borderRadius: 8, fontFamily: "'Outfit', sans-serif", fontSize: 14, fontWeight: 500, color: '#1A1500', textAlign: 'center', outline: 'none' }}
                    onFocus={e => e.target.style.borderColor = '#ffd43a'}
                    onBlur={e => e.target.style.borderColor = '#F0E4C8'}
                  />
                </div>
                {/* Toggle */}
                <button onClick={() => handleToggle(item.id)} style={{ display: 'flex', alignItems: 'center', gap: 6, background: item.available ? '#EDFBF3' : '#FFF8E7', border: 'none', borderRadius: 20, padding: '6px 12px', cursor: 'pointer', fontFamily: "'Outfit', sans-serif", fontSize: 12, fontWeight: 500, color: item.available ? '#1a7a4a' : '#8B7355', whiteSpace: 'nowrap' }}>
...
function StatsSection({ orders }) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 12, marginBottom: 24 }}>
      {stats.map(s => (
        <div key={s.label} style={{ background: '#fff', border: '1px solid #F0E4C8', borderRadius: 14, padding: '16px 18px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            <span style={{ fontSize: 18 }}>{s.icon}</span>
            <span style={{ fontSize: 11, color: '#8B7355', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '1px' }}>{s.label}</span>
          </div>
          <p style={{ fontFamily: "'Fraunces', serif", fontSize: 28, fontWeight: 600, color: s.color, margin: '0 0 3px', lineHeight: 1 }}>{s.value}</p>
          <p style={{ fontSize: 12, color: '#8B7355', margin: 0 }}>{s.sub}</p>
        </div>
      ))}
    </div>
  );
...
export default function AdminPage() {
  ...
  return (
    <div style={{ fontFamily: "'Outfit', sans-serif", background: '#FFF8E7', minHeight: '100vh' }}>
      ...
      <div style={{ background: '#1A1500', padding: '0 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 56, position: 'sticky', top: 0, zIndex: 50 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontFamily: "'Fraunces', serif", fontSize: 16, fontWeight: 600, color: '#FFF8DC' }}>OhMyGrill</span>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px 20px' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 16, overflowX: 'auto', paddingBottom: 4 }}>
              {[{ id: 'all', label: 'Todos' }, ...Object.entries(STATUS_CONFIG).map(([id, cfg]) => ({ id, label: cfg.label }))].map(f => (
                <button key={f.id} onClick={() => setFilterStatus(f.id)} style={{ background: filterStatus === f.id ? '#1A1500' : '#fff', color: filterStatus === f.id ? '#fff' : '#7A6030', border: '1px solid #F0E4C8', borderRadius: 20, padding: '7px 16px', fontFamily: "'Outfit', sans-serif", fontSize: 13, cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}>
        ...
        {tab === 'stats' && (
          <div>
            ...
            {orders.length > 0 && <>
            {/* Revenue by zone */}
            <div style={{ background: '#fff', border: '1px solid #F0E4C8', borderRadius: 20, padding: '24px', marginBottom: 16 }}>
              ...
              {Object.entries(
                ...
                return (
                  <div key={zone} style={{ marginBottom: 12 }}>
                    ...
                    <div style={{ height: 6, background: '#FFF8E7', borderRadius: 3, overflow: 'hidden' }}>
            ...
            {/* Top products */}
            <div style={{ background: '#fff', border: '1px solid #F0E4C8', borderRadius: 20, padding: '24px' }}>
              ...
              {Object.entries(
                ...
                <div key={name} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '9px 0', borderBottom: i < 5 ? '1px solid #FFF8E7' : 'none' }}>
