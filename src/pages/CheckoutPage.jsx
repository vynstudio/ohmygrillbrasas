          ...
          <div key={s} style={{ display:'flex', alignItems:'center', flex: i < steps.length - 1 ? 1 : 'none' }}>
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <div style={{
                ...
                background: done ? '#1a7a4a' : active ? '#ffd43a' : '#3D2200',
              ...
              <span style={{ fontSize:13, fontWeight: active ? 500 : 400, color: active ? '#1A1500' : done ? '#1a7a4a' : '#C4A265', transition:'all 0.3s', whiteSpace:'nowrap' }}>
                {s}
              </span>
            </div>
            {i < steps.length - 1 && (
              <div style={{ flex:1, height:1, background: done ? '#1a7a4a' : '#3D2200', margin:'0 12px', transition:'background 0.3s' }} />
            )}
          </div>
        );
      })}
  ...
  return (
    <div style={{ background:'#2A1800', border:'1px solid #3D2200', borderRadius:20, overflow:'hidden' }}>
      <div style={{ background:'#1A1500', padding:'18px 22px' }}>
        <p style={{ fontFamily:"'Fraunces',serif", fontSize:17, fontWeight:600, color:'#F5DFA0', margin:0 }}>Resumen del pedido</p>
      </div>
      <div style={{ padding:'16px 22px' }}>
        {items.map(item => (
          <div key={item.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 0', borderBottom:'1px solid #2A1800' }}>
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <p style={{ fontSize:11, color:'#C4A265', margin:'2px 0 0' }}>€{(item.price % 1 === 0 ? item.price : item.price.toFixed(2))} / ud.</p>
            </div>
            <div style={{ display:'flex', alignItems:'center', gap:7, flexShrink:0 }}>
              <button onClick={() => updateQty(item.id, item.qty - 1)} style={{ width:22, height:22, border:'1px solid #D4CFC9', background:'#2A1800', borderRadius:'50%', cursor:'pointer', fontSize:13, color:'#C4A265', display:'flex', alignItems:'center', justifyContent:'center' }}>−</button>
        ...
        <div style={{ paddingTop:12 }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:12, paddingBottom:12, borderBottom:'1px solid #F0EDE8' }}>
            <span style={{ fontSize:13, color:'#C4A265' }}>Envío {deliveryType === 'pickup' ? '(recogida)' : deliveryZone ? `· ${deliveryZone.name}` : ''}</span>
      ...
      <div style={{ borderTop:'1px solid #F0EDE8', padding:'12px 22px', background:'#1C0F00' }}>
        {[{ icon:'🔒', text:'Pago seguro con Stripe' }, { icon:'🛵', text:'Entrega en 90 min · Zaragoza' }].map(i => (
          <div key={i.text} style={{ display:'flex', alignItems:'center', gap:8, marginBottom:6 }}>
            ...
            <span style={{ fontSize:12, color:'#C4A265' }}>{i.text}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:24 }}>
        {[{ id:'delivery', icon:'🛵', title:'A domicilio', desc:'Entrega en tu dirección' }, { id:'pickup', icon:'🏪', title:'Recogida', desc:'Recoge en el local' }].map(opt => (
          <button key={opt.id} onClick={() => setDeliveryType(opt.id)} style={{ padding:'14px 16px', border:`2px solid ${deliveryType===opt.id ? '#ffd43a' : '#3D2200'}`, borderRadius:14, background: deliveryType===opt.id ? '#FEF3EE' : '#fff', cursor:'pointer', textAlign:'left', transition:'all 0.15s' }}>
            ...
            <p style={{ fontSize:12, color:'#C4A265', margin:0 }}>{opt.desc}</p>
          </button>
        ))}
      </div>
      ...
      {deliveryType === 'delivery' && (
        <>
          ...
          <div style={{ marginBottom:16 }}>
            ...
            <select value={deliveryZone?.name || ''} onChange={e => { const z = deliveryZones.find(z => z.name === e.target.value); setDeliveryZone(z || null); }} style={{ width:'100%', padding:'11px 14px', border:`1.5px solid ${errors.zone ? '#E24B4A' : '#3D2200'}`, borderRadius:12, fontFamily:"'Outfit',sans-serif", fontSize:14, color:'#1A1500', background:'#2A1800', cursor:'pointer' }}>
          ...
          <div style={{ marginBottom:16 }}>
            ...
            <input type="text" placeholder="Calle, número, piso..." value={data.address || ''} onChange={e => onChange('address', e.target.value)} style={{ width:'100%', padding:'11px 14px', border:`1.5px solid ${errors.address ? '#E24B4A' : '#3D2200'}`, borderRadius:12, fontFamily:"'Outfit',sans-serif", fontSize:14, color:'#1A1500', boxSizing:'border-box', outline:'none' }} onFocus={e => e.target.style.borderColor='#ffd43a'} onBlur={e => e.target.style.borderColor= errors.address ? '#E24B4A' : '#3D2200'} />
      ...
      {deliveryType === 'pickup' && (
        <div style={{ background:'#FEF3EE', border:'1px solid rgba(232,88,32,0.2)', borderRadius:12, padding:'14px 16px', marginBottom:16 }}>
          ...
          <p style={{ fontSize:13, color:'#C4A265', margin:0 }}>Calle de las Brasas, 12 · 50001 Zaragoza</p>
          <p style={{ fontSize:12, color:'#C4A265', margin:'6px 0 0' }}>Listo en ~25 minutos desde la confirmación</p>
        </div>
      )}

      {/* Notes */}
      <div style={{ marginBottom:24 }}>
        <label style={{ fontSize:13, fontWeight:500, color:'#1A1500', display:'block', marginBottom:6 }}>Notas del pedido <span style={{ color:'#C4A265', fontWeight:400 }}>(opcional)</span></label>
        <textarea placeholder="Punto de la carne, alergias, instrucciones especiales..." value={notes} onChange={e => setNotes(e.target.value)} rows={3} style={{ width:'100%', padding:'11px 14px', border:'1.5px solid #3D2200', borderRadius:12, fontFamily:"'Outfit',sans-serif", fontSize:14, color:'#1A1500', resize:'vertical', boxSizing:'border-box', outline:'none' }} onFocus={e => e.target.style.borderColor='#ffd43a'} onBlur={e => e.target.style.borderColor='#3D2200'} />
      </div>
...
function StepContact({ data, onChange, onNext, onBack }) {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize:13, color:'#C4A265', margin:'0 0 24px' }}>Te enviaremos la confirmación por email y te avisaremos cuando salga tu pedido.</p>
      ...
      {fields.map(f => (
        <div key={f.key} style={{ marginBottom:16 }}>
          ...
          <input type={f.type} placeholder={f.placeholder} value={data[f.key] || ''} onChange={e => onChange(f.key, e.target.value)}
            style={{ width:'100%', padding:'11px 14px', border:`1.5px solid ${errors[f.key] ? '#E24B4A' : '#3D2200'}`, borderRadius:12, fontFamily:"'Outfit',sans-serif", fontSize:14, color:'#1A1500', boxSizing:'border-box', outline:'none' }}
            onFocus={e => e.target.style.borderColor='#ffd43a'} onBlur={e => e.target.style.borderColor = errors[f.key] ? '#E24B4A' : '#3D2200'} />
          {errors[f.key] && <p style={{ fontSize:12, color:'#E24B4A', marginTop:4 }}>{errors[f.key]}</p>}
        </div>
      ))}

      <div style={{ background:'#2A1800', borderRadius:12, padding:'12px 14px', marginBottom:24, display:'flex', gap:10, alignItems:'flex-start' }}>
        <span style={{ fontSize:16, flexShrink:0 }}>🔒</span>
        <p style={{ fontSize:12, color:'#C4A265', margin:0, lineHeight:1.5 }}>Tus datos solo se usan para gestionar el pedido. Nunca los compartimos con terceros.</p>
      </div>

      <div style={{ display:'grid', gridTemplateColumns:'1fr 2fr', gap:10 }}>
        <button onClick={onBack} style={{ background:'transparent', color:'#C4A265', border:'1.5px solid #3D2200', borderRadius:14, padding:'14px', fontFamily:"'Outfit',sans-serif", fontSize:14, fontWeight:500, cursor:'pointer' }}>
          ← Atrás
        </button>
...
function StepPayment({ contactData, onBack, onSuccess }) {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize:13, color:'#C4A265', margin:'0 0 24px' }}>Procesado por Stripe · Cifrado SSL · No almacenamos datos de tarjeta</p>

      {/* Accepted cards */}
      <div style={{ display:'flex', gap:8, alignItems:'center', marginBottom:20 }}>
        {['VISA', 'MC', 'AMEX'].map(c => (
          <div key={c} style={{ background:'#2A1800', border:'1px solid #3D2200', borderRadius:6, padding:'4px 10px', fontSize:11, fontWeight:600, color:'#C4A265', letterSpacing:'0.5px' }}>{c}</div>
        ))}
        <span style={{ fontSize:12, color:'#C4A265' }}>+ Bizum próximamente</span>
      </div>

      {/* Stripe Elements card */}
      <div style={{ background:'#1C0F00', border:'1px solid #3D2200', borderRadius:16, padding:'20px', marginBottom:20 }}>
        <label style={{ fontSize:13, fontWeight:500, color:'#1A1500', display:'block', marginBottom:10 }}>Datos de tarjeta</label>
        <div ref={cardElementRef} style={{ background:'#2A1800', border:'1.5px solid #3D2200', borderRadius:12, padding:'13px 14px', minHeight:44 }} />
        {!cardReady && <p style={{ fontSize:12, color:'#C4A265', marginTop:6 }}>Cargando formulario seguro...</p>}
      </div>
      ...
      <div style={{ background:'#1A1500', borderRadius:14, padding:'16px 20px', marginBottom:12 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <div>
            ...
            <p style={{ fontFamily:"'Fraunces',serif", fontSize:24, fontWeight:600, color:'#F5DFA0', margin:'2px 0 0' }}>€{total.toFixed(2)}</p>
          </div>
          <button onClick={handlePay} disabled={loading} style={{ background: loading ? '#C4A265' : '#ffd43a', color:'#fff', border:'none', borderRadius:12, padding:'13px 24px', fontFamily:"'Outfit',sans-serif", fontSize:15, fontWeight:500, cursor: loading ? 'not-allowed' : 'pointer', transition:'all 0.2s', display:'flex', alignItems:'center', gap:8, minWidth:140, justifyContent:'center' }}>
            {loading ? (
              <>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 2fr', gap:10 }}>
        <button onClick={onBack} style={{ background:'transparent', color:'#C4A265', border:'1.5px solid #3D2200', borderRadius:14, padding:'13px', fontFamily:"'Outfit',sans-serif", fontSize:14, cursor:'pointer' }}>
          ← Atrás
        </button>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:6 }}>
          <span style={{ fontSize:11 }}>🔒</span>
          <span style={{ fontSize:11, color:'#C4A265' }}>Cifrado SSL · Procesado por Stripe</span>
        </div>
      </div>
    </div>
  );
...
export default function CheckoutPage({ onNavigate }) {
  ...
  if (itemCount === 0 && !orderResult) {
    return (
      <div style={{ minHeight:'60vh', display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', gap:16, fontFamily:"'Outfit',sans-serif", padding:'40px 24px', textAlign:'center' }}>
        ...
        <p style={{ fontSize:14, color:'#C4A265' }}>Añade platos de la carta antes de pagar.</p>
    ...
    return (
      <div style={{ minHeight:'80vh', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"'Outfit',sans-serif", padding:'40px 24px' }}>
        <div style={{ maxWidth:520, width:'100%', textAlign:'center' }}>
          ...
          <p style={{ fontSize:14, color:'#C4A265', lineHeight:1.65, marginBottom:24 }}>
          ...
          <div style={{ background:'#2A1800', border:'1px solid #3D2200', borderRadius:16, padding:'20px 24px', marginBottom:24, textAlign:'left' }}>
          ...
          <div style={{ background:'#FEF3EE', border:'1px solid rgba(232,88,32,0.2)', borderRadius:12, padding:'14px 16px', marginBottom:28, display:'flex', gap:10, alignItems:'center' }}>
            ...
            <p style={{ fontSize:13, color:'#C4A265', margin:0, lineHeight:1.5 }}>
          ...
          <button onClick={() => onNavigate('home')} style={{ width:'100%', background:'transparent', color:'#C4A265', border:'none', padding:'12px', fontFamily:"'Outfit',sans-serif", fontSize:14, cursor:'pointer' }}>
            Volver al inicio
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{ fontFamily:"'Outfit',sans-serif", background:'#1C0F00', minHeight:'100vh' }}>
      {/* Header */}
      <div style={{ background:'#2A1800', borderBottom:'1px solid #3D2200', padding: isMobile ? '24px 20px 20px' : '32px 0 24px' }}>
        <div style={{ maxWidth:1100, margin:'0 auto', padding: isMobile ? '0' : '0 24px' }}>
          <button onClick={() => step === 0 ? onNavigate('menu') : setStep(s => s - 1)} style={{ background:'none', border:'none', cursor:'pointer', fontSize:13, color:'#C4A265', fontFamily:"'Outfit',sans-serif", padding:'0 0 12px', display:'flex', alignItems:'center', gap:6 }}>
      ...
      <div style={{ maxWidth:1100, margin:'0 auto', padding: isMobile ? '24px 20px' : '32px 24px', display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 360px', gap:32, alignItems:'start' }}>
        ...
        {/* Left — step form */}
        <div style={{ background:'#2A1800', borderRadius:20, border:'1px solid #3D2200', padding: isMobile ? '24px 20px' : '32px' }}>
          {step === 0 && (
        ...
        {isMobile && (
          <details style={{ background:'#2A1800', borderRadius:16, border:'1px solid #3D2200', overflow:'hidden' }}>
