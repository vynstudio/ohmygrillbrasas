          ...
          <div key={stage.id} style={{ display: 'flex', gap: 16, marginBottom: idx < stages.length - 1 ? 0 : 0 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
              <div style={{
                ...
                background: done ? '#EDFBF3' : active ? '#1C1A14' : '#F5F1EC',
                border: `2px solid ${done ? '#1a7a4a' : active ? '#1C1A14' : '#EDE9E3'}`,
            ...
            <div style={{ paddingBottom: idx < stages.length - 1 ? 28 : 0, paddingTop: 10, flex: 1 }}>
              <p style={{
                ...
                color: done ? '#1a7a4a' : active ? '#1C1A14' : '#B8AFA8',
                margin: '0 0 3px',
                transition: 'all 0.3s',
              }}>
                {stage.label}
              ...
              {active && (
                <p style={{ fontSize: 13, color: '#7A6E63', margin: 0, lineHeight: 1.5 }}>{stage.desc}</p>
              )}
            </div>
          </div>
        );
    ...
    <div style={{ fontFamily: "'Outfit', sans-serif", background: '#FAFAF5', minHeight: '100vh' }}>
      ...
      <div style={{ background: '#1C1A14', padding: isMobile ? '48px 20px 40px' : '64px 0 56px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 680, margin: '0 auto', padding: isMobile ? '0' : '0 24px', position: 'relative', zIndex: 1 }}>
          <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: isMobile ? 32 : 44, fontWeight: 600, color: '#F0EBE3', margin: '0 0 10px', letterSpacing: '-1px' }}>
        ...
        <div style={{ maxWidth: 680, margin: '0 auto', padding: isMobile ? '28px 20px' : '40px 24px' }}>
          ...
          <div style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ fontSize: 11, color: isDelivered ? '#1a7a4a' : '#9A8F85', fontWeight: 600, letterSpacing: '1.5px', textTransform: 'uppercase', margin: '0 0 4px' }}>
                {order.id}
              </p>
              <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: 22, fontWeight: 600, color: isDelivered ? '#1a7a4a' : '#1C1A14', margin: '0 0 4px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 16, marginBottom: 20 }}>
            ...
            <div style={{ background: '#fff', border: '1px solid #EDE9E3', borderRadius: 20, padding: '24px' }}>
              <p style={{ fontSize: 11, letterSpacing: '2px', color: '#9A8F85', fontWeight: 600, textTransform: 'uppercase', margin: '0 0 20px' }}>Estado del pedido</p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {/* Items */}
              <div style={{ background: '#fff', border: '1px solid #EDE9E3', borderRadius: 20, padding: '20px', flex: 1 }}>
                <p style={{ fontSize: 11, letterSpacing: '2px', color: '#9A8F85', fontWeight: 600, textTransform: 'uppercase', margin: '0 0 12px' }}>Artículos</p>
                {order.items.map((item, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: i < order.items.length - 1 ? '1px solid #F5F1EC' : 'none' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <p style={{ fontSize: 13, fontWeight: 500, color: '#1C1A14', margin: 0 }}>{item.qty}× {item.name}</p>
                    </div>
                    <span style={{ fontSize: 13, color: '#7A6E63' }}>€{(item.price * item.qty).toFixed(2)}</span>
                  </div>
                ))}
              ...
              {/* Delivery info */}
              <div style={{ background: '#fff', border: '1px solid #EDE9E3', borderRadius: 16, padding: '16px 18px' }}>
                <p style={{ fontSize: 11, letterSpacing: '2px', color: '#9A8F85', fontWeight: 600, textTransform: 'uppercase', margin: '0 0 10px' }}>
                  {order.deliveryType === 'pickup' ? 'Recogida' : 'Entrega'}
                </p>
                {order.deliveryType === 'delivery' ? (
                  <>
                    <p style={{ fontSize: 13, color: '#1C1A14', fontWeight: 500, margin: '0 0 3px' }}>📍 {order.address}</p>
                    <p style={{ fontSize: 12, color: '#9A8F85', margin: 0 }}>{order.zone}</p>
                  </>
                ) : (
                  <>
                    <p style={{ fontSize: 13, color: '#1C1A14', fontWeight: 500, margin: '0 0 3px' }}>🏪 Recogida en local</p>
                    <p style={{ fontSize: 12, color: '#9A8F85', margin: 0 }}>Calle de las Brasas, 12 · Zaragoza</p>
                  </>
                )}
              </div>
            </div>
          ...
          {/* Help bar */}
          <div style={{ background: '#1C1A14', borderRadius: 16, padding: '18px 22px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
            <div>
              <p style={{ fontSize: 14, fontWeight: 500, color: '#F0EBE3', margin: '0 0 2px' }}>¿Tienes algún problema?</p>
              <p style={{ fontSize: 12, color: 'rgba(240,235,227,0.4)', margin: 0 }}>Estamos disponibles durante el horario de reparto</p>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <a href={`tel:${order.phone}`} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(255,255,255,0.08)', color: '#F0EBE3', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, padding: '9px 14px', fontFamily: "'Outfit', sans-serif", fontSize: 13, textDecoration: 'none', whiteSpace: 'nowrap' }}>
                📞 Llamar
              </a>
      ...
      {!order && !loading && !error && (
        <div style={{ maxWidth: 680, margin: '0 auto', padding: isMobile ? '40px 20px' : '56px 24px', textAlign: 'center' }}>
          ...
          <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: 24, fontWeight: 600, color: '#1C1A14', margin: '0 0 10px' }}>
            Encuentra tu pedido
          </h2>
          <p style={{ fontSize: 14, color: '#9A8F85', lineHeight: 1.65, maxWidth: 380, margin: '0 auto 28px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, maxWidth: 400, margin: '0 auto' }}>
            {[
              ...
              <div key={item.text} style={{ background: '#fff', border: '1px solid #EDE9E3', borderRadius: 12, padding: '14px 10px', textAlign: 'center' }}>
                <div style={{ fontSize: 22, marginBottom: 6 }}>{item.icon}</div>
                <p style={{ fontSize: 11, color: '#7A6E63', margin: 0, lineHeight: 1.4 }}>{item.text}</p>
              </div>
            ))}
          </div>
        </div>
