          ...
          <div key={stage.id} style={{ display: 'flex', gap: 16, marginBottom: idx < stages.length - 1 ? 0 : 0 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
              <div style={{
                ...
                background: done ? '#EDFBF3' : active ? '#1A0800' : '#2C1200',
                border: `2px solid ${done ? '#1a7a4a' : active ? '#1A0800' : '#3D1800'}`,
            ...
            <div style={{ paddingBottom: idx < stages.length - 1 ? 28 : 0, paddingTop: 10, flex: 1 }}>
              <p style={{
                ...
                color: done ? '#1a7a4a' : active ? '#1A0800' : '#D4956A',
                margin: '0 0 3px',
                transition: 'all 0.3s',
              }}>
                {stage.label}
              ...
              {active && (
                <p style={{ fontSize: 13, color: '#D4956A', margin: 0, lineHeight: 1.5 }}>{stage.desc}</p>
              )}
            </div>
          </div>
        );
...
export default function OrderTrackerPage({ onNavigate }) {
  ...
  return (
    <div style={{ fontFamily: "'Outfit', sans-serif", background: '#1A0800', minHeight: '100vh' }}>
      ...
      {/* Header */}
      <div style={{ background: '#1A0800', padding: isMobile ? '48px 20px 40px' : '64px 0 56px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 680, margin: '0 auto', padding: isMobile ? '0' : '0 24px', position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button
              ...
              disabled={loading}
              style={{
                background: loading ? '#D4956A' : '#ffd43a',
      ...
      {order && (
        <div style={{ maxWidth: 680, margin: '0 auto', padding: isMobile ? '28px 20px' : '40px 24px' }}>
          ...
          <div style={{
            ...
            border: `1px solid ${isDelivered ? '#1a7a4a' : isReady ? '#ffd43a' : '#3D1800'}`,
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ fontSize: 11, color: isDelivered ? '#1a7a4a' : '#D4956A', fontWeight: 600, letterSpacing: '1.5px', textTransform: 'uppercase', margin: '0 0 4px' }}>
                {order.id}
              </p>
              <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: 22, fontWeight: 600, color: isDelivered ? '#1a7a4a' : '#1A0800', margin: '0 0 4px' }}>
                {stages[currentStageIdx]?.label}
              </h2>
              <p style={{ fontSize: 13, color: '#D4956A', margin: 0 }}>
            ...
            <div style={{ textAlign: 'right', flexShrink: 0 }}>
              ...
              <p style={{ fontSize: 11, color: '#D4956A', margin: '3px 0 0' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 16, marginBottom: 20 }}>
            ...
            <div style={{ background: '#fff', border: '1px solid #2A1A00', borderRadius: 20, padding: '24px' }}>
              <p style={{ fontSize: 11, letterSpacing: '2px', color: '#D4956A', fontWeight: 600, textTransform: 'uppercase', margin: '0 0 20px' }}>Estado del pedido</p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {/* Items */}
              <div style={{ background: '#fff', border: '1px solid #2A1A00', borderRadius: 20, padding: '20px', flex: 1 }}>
                <p style={{ fontSize: 11, letterSpacing: '2px', color: '#D4956A', fontWeight: 600, textTransform: 'uppercase', margin: '0 0 12px' }}>Artículos</p>
                {order.items.map((item, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: i < order.items.length - 1 ? '1px solid #1A1000' : 'none' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <p style={{ fontSize: 13, fontWeight: 500, color: '#1A0800', margin: 0 }}>{item.qty}× {item.name}</p>
                    </div>
                    <span style={{ fontSize: 13, color: '#D4956A' }}>€{(item.price * item.qty).toFixed(2)}</span>
                  </div>
                ))}
                <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 12, borderTop: '1px solid #2A1A00', marginTop: 8 }}>
                  <span style={{ fontFamily: "'Fraunces', serif", fontSize: 15, fontWeight: 600, color: '#1A0800' }}>Total</span>
                  <span style={{ fontFamily: "'Fraunces', serif", fontSize: 15, fontWeight: 600, color: '#ffd43a' }}>€{order.total.toFixed(2)}</span>
                </div>
              </div>

              {/* Delivery info */}
              <div style={{ background: '#fff', border: '1px solid #2A1A00', borderRadius: 16, padding: '16px 18px' }}>
                <p style={{ fontSize: 11, letterSpacing: '2px', color: '#D4956A', fontWeight: 600, textTransform: 'uppercase', margin: '0 0 10px' }}>
                  {order.deliveryType === 'pickup' ? 'Recogida' : 'Entrega'}
                </p>
                {order.deliveryType === 'delivery' ? (
                  <>
                    <p style={{ fontSize: 13, color: '#1A0800', fontWeight: 500, margin: '0 0 3px' }}>📍 {order.address}</p>
                    <p style={{ fontSize: 12, color: '#D4956A', margin: 0 }}>{order.zone}</p>
                  </>
                ) : (
                  <>
                    <p style={{ fontSize: 13, color: '#1A0800', fontWeight: 500, margin: '0 0 3px' }}>🏪 Recogida en local</p>
                    <p style={{ fontSize: 12, color: '#D4956A', margin: 0 }}>Calle de las Brasas, 12 · Zaragoza</p>
                  </>
                )}
              </div>
            </div>
          ...
          {/* Help bar */}
          <div style={{ background: '#1A0800', borderRadius: 16, padding: '18px 22px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
            <div>
          ...
          {isDelivered && (
            <div style={{ marginTop: 20, textAlign: 'center' }}>
              <p style={{ fontSize: 14, color: '#D4956A', marginBottom: 12 }}>¿Te ha gustado? ¡Repite!</p>
      ...
      {!order && !loading && !error && (
        <div style={{ maxWidth: 680, margin: '0 auto', padding: isMobile ? '40px 20px' : '56px 24px', textAlign: 'center' }}>
          ...
          <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: 24, fontWeight: 600, color: '#1A0800', margin: '0 0 10px' }}>
            Encuentra tu pedido
          </h2>
          <p style={{ fontSize: 14, color: '#D4956A', lineHeight: 1.65, maxWidth: 380, margin: '0 auto 28px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, maxWidth: 400, margin: '0 auto' }}>
            {[
              ...
              <div key={item.text} style={{ background: '#fff', border: '1px solid #2A1A00', borderRadius: 12, padding: '14px 10px', textAlign: 'center' }}>
                <div style={{ fontSize: 22, marginBottom: 6 }}>{item.icon}</div>
                <p style={{ fontSize: 11, color: '#D4956A', margin: 0, lineHeight: 1.4 }}>{item.text}</p>
              </div>
            ))}
          </div>
        </div>
