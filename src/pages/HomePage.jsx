...
export default function HomePage({ onNavigate }) {
  ...
  return (
    <div style={{ fontFamily:"'Outfit',sans-serif", background:'#FAFAF7' }}>
        ...
        <div style={{ position:'relative', zIndex:2, maxWidth:1200, margin:'0 auto', padding: isDesktop ? '80px 64px' : isMobile ? '40px 20px 48px' : '60px 32px', display:'grid', gridTemplateColumns: isDesktop ? '1fr 420px' : '1fr', gap: isDesktop ? 64 : 0, alignItems:'center', width:'100%' }}>
          ...
          <div style={{ opacity: heroVisible ? 1 : 0, transform: heroVisible ? 'translateY(0)' : 'translateY(28px)', transition:'all 0.8s cubic-bezier(0.4,0,0.2,1)' }}>
            ...
            <div style={{ display:'flex', gap:12, flexDirection: isMobile ? 'column' : 'row' }}>
              <button onClick={() => onNavigate('menu')} style={{ background:'#E85820', color:'#fff', border:'none', borderRadius:32, padding: isMobile ? '16px 24px' : '15px 32px', fontSize:15, fontWeight:500, cursor:'pointer', boxShadow:'0 4px 28px rgba(232,88,32,0.45)', transition:'all 0.2s', fontFamily:"'Outfit',sans-serif", textAlign:'center' }}>
                Ver la carta →
              </button>
            ...
            <div style={{ opacity: heroVisible ? 1 : 0, transform: heroVisible ? 'translateY(0)' : 'translateY(32px)', transition:'all 0.9s cubic-bezier(0.4,0,0.2,1) 0.2s' }}>
              <div style={{ background:'rgba(20,16,10,0.75)', backdropFilter:'blur(24px)', border:'1px solid rgba(255,255,255,0.10)', borderRadius:24, padding:28, boxShadow:'0 32px 80px rgba(0,0,0,0.5)' }}>
                ...
                <div style={{ background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:16, padding:'18px 20px', marginBottom:14 }}>
                  ...
                  <button onClick={() => addItem(products[0])} style={{ width:'100%', background:'#E85820', color:'#fff', border:'none', borderRadius:10, padding:'11px', fontFamily:"'Outfit',sans-serif", fontSize:14, fontWeight:500, cursor:'pointer' }}>
                    Añadir al pedido
                  </button>
                </div>
      ...
      <section style={{ background:'#fff', padding: isMobile ? '48px 0' : '72px 0' }}>
        <div style={{ maxWidth:1200, margin:'0 auto', padding:`0 ${isMobile ? 20 : 24}px` }}>
          <div style={{ display:'flex', alignItems:'baseline', justifyContent:'space-between', marginBottom: isMobile ? 24 : 36 }}>
            ...
            <button onClick={() => onNavigate('menu')} style={{ background:'none', border:'none', cursor:'pointer', fontSize:13, color:'#E85820', fontFamily:"'Outfit',sans-serif", fontWeight:500, textDecoration:'underline', textUnderlineOffset:3, whiteSpace:'nowrap' }}>
              Ver todo →
            </button>
          </div>
      ...
      {/* ── PACKS ── */}
      <section style={{ padding: isMobile ? '48px 0' : '72px 0', background:'#FAFAF7' }}>
        <div style={{ maxWidth:1200, margin:'0 auto', padding:`0 ${isMobile ? 20 : 24}px` }}>
            ...
            {packs.map((pack, idx) => (
              <div key={pack.id} style={{ background: idx===2 ? '#1C1A14' : '#fff', borderRadius:20, padding: isMobile ? '22px 20px' : '28px 24px', border: idx===2 ? 'none' : '1px solid #EDE9E3', position:'relative', overflow:'hidden' }}>
                <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:14 }}>
                  <div>
                    <span style={{ display:'inline-block', background: idx===2 ? '#E85820' : '#FEF3EE', color: idx===2 ? '#fff' : '#E85820', fontSize:11, fontWeight:600, padding:'3px 10px', borderRadius:20, marginBottom:8 }}>{pack.badge}</span>
                ...
                <ul style={{ margin:'0 0 16px', padding:0, listStyle:'none' }}>
                  {pack.items.map(item => (
                    <li key={item} style={{ fontSize:13, color: idx===2 ? 'rgba(240,235,227,0.65)' : '#7A6E63', padding:'3px 0', display:'flex', alignItems:'center', gap:8 }}>
                      <span style={{ color:'#E85820', fontSize:10 }}>●</span>{item}
                    </li>
                  ))}
                </ul>
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                  ...
                  <button onClick={() => onNavigate('packs')} style={{ background: idx===2 ? '#E85820' : '#1C1A14', color:'#fff', border:'none', borderRadius:24, padding:'10px 20px', fontSize:13, fontWeight:500, cursor:'pointer', fontFamily:"'Outfit',sans-serif" }}>
                    Pedir pack
                  </button>
                </div>
              </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3,1fr)', gap: isMobile ? 28 : 40 }}>
            {[
              ...
              <div key={s.step} style={{ display:'flex', flexDirection: isMobile ? 'row' : 'column', alignItems: isMobile ? 'flex-start' : 'center', gap: isMobile ? 16 : 0, textAlign: isMobile ? 'left' : 'center' }}>
                ...
                <div>
                  <span style={{ fontSize:11, letterSpacing:'2px', color:'#E85820', fontWeight:600, display:'block', marginBottom:4 }}>{s.step}</span>
      ...
      <section style={{ background:'#1C1A14', padding: isMobile ? '56px 20px' : '80px 24px' }}>
        <div style={{ maxWidth:800, margin:'0 auto', textAlign:'center' }}>
          ...
          <button onClick={() => onNavigate('menu')} style={{ background:'#E85820', color:'#fff', border:'none', borderRadius:32, padding: isMobile ? '15px 32px' : '16px 40px', fontSize: isMobile ? 15 : 16, fontWeight:500, cursor:'pointer', fontFamily:"'Outfit',sans-serif", boxShadow:'0 4px 28px rgba(232,88,32,0.4)', width: isMobile ? '100%' : 'auto' }}>
            Pedir ahora →
          </button>
        </div>
      </section>
      ...
      <footer style={{ background:'#111', padding: isMobile ? '40px 20px 28px' : '52px 24px 32px' }}>
        <div style={{ maxWidth:1200, margin:'0 auto' }}>
          <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : '2fr 1fr 1fr 1fr', gap: isMobile ? 28 : 40, marginBottom: isMobile ? 28 : 40 }}>
            ...
            <div>
              <p style={{ fontSize:11, letterSpacing:'2px', color:'#E85820', marginBottom:12, fontWeight:600, textTransform:'uppercase' }}>Carta</p>
            ...
            <div>
              <p style={{ fontSize:11, letterSpacing:'2px', color:'#E85820', marginBottom:12, fontWeight:600, textTransform:'uppercase' }}>Info</p>
            ...
            {!isMobile && (
              <div>
                <p style={{ fontSize:11, letterSpacing:'2px', color:'#E85820', marginBottom:12, fontWeight:600, textTransform:'uppercase' }}>Horario</p>
