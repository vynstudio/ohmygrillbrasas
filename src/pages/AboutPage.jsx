...
export default function AboutPage({ onNavigate }) {
  ...
  const inputStyle = (hasError) => ({
    ...
    border: `1.5px solid ${hasError ? '#E24B4A' : '#3D2200'}`,
    ...
    <div style={{ fontFamily: "'Outfit', sans-serif", background: '#1C0F00', minHeight: '100vh' }}>
      ...
      <section style={{ background: '#1A1500', padding: isMobile ? '56px 20px 48px' : '80px 0 72px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: isMobile ? '0' : '0 24px', position: 'relative', zIndex: 1 }}>
          ...
          <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: isMobile ? 38 : 58, fontWeight: 600, color: '#F5DFA0', margin: '0 0 20px', lineHeight: 1.0, letterSpacing: '-1px' }}>
            Más de diez años<br />
      ...
      <section style={{ background: '#fff', padding: isMobile ? '48px 20px' : '72px 0' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: isMobile ? '0' : '0 24px', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 32 : 64, alignItems: 'center' }}>
          <div>
            ...
            <p style={{ fontSize: 15, color: '#C4A265', lineHeight: 1.8, margin: '0 0 16px' }}>
            ...
            <p style={{ fontSize: 15, color: '#C4A265', lineHeight: 1.8, margin: 0 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            {[
              ...
              <div key={s.label} style={{ background: '#1C0F00', border: '1px solid #3D2200', borderRadius: 16, padding: '22px 18px' }}>
                ...
                <p style={{ fontSize: 11, color: '#C4A265', margin: 0 }}>{s.sub}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── VALUES ── */}
      <section style={{ background: '#1C0F00', padding: isMobile ? '48px 20px' : '72px 0' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: isMobile ? '0' : '0 24px' }}>
          ...
          <p style={{ fontSize: 14, color: '#C4A265', textAlign: 'center', margin: '0 0 36px' }}>Por qué nuestros clientes llevan más de diez años volviendo</p>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(4,1fr)', gap: 16 }}>
            {values.map(v => (
              <div key={v.title} style={{ background: '#fff', border: '1px solid #3D2200', borderRadius: 18, padding: '24px 20px' }}>
                ...
                <p style={{ fontSize: 13, color: '#C4A265', lineHeight: 1.65, margin: 0 }}>{v.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ background: '#fff', padding: isMobile ? '48px 20px' : '72px 0' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: isMobile ? '0' : '0 24px' }}>
          ...
          <p style={{ fontSize: 14, color: '#C4A265', textAlign: 'center', margin: '0 0 36px' }}>Las personas detrás de cada brasa</p>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3,1fr)', gap: 20 }}>
            {team.map(member => (
              <div key={member.name} style={{ background: '#1C0F00', border: '1px solid #3D2200', borderRadius: 20, padding: '28px 24px', textAlign: 'center' }}>
                ...
                <p style={{ fontSize: 12, color: '#C4A265', margin: 0 }}>{member.years}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── CONTACT ── */}
      <section id="contact" style={{ background: '#1C0F00', padding: isMobile ? '48px 20px' : '72px 0' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: isMobile ? '0' : '0 24px', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 40 : 64 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 15, color: '#C4A265', lineHeight: 1.75, margin: '0 0 36px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {[
                ...
                <div key={item.label} style={{ background: '#fff', border: '1px solid #3D2200', borderRadius: 14, padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 14 }}>
                  ...
                  <div>
                    <p style={{ fontSize: 11, color: '#C4A265', fontWeight: 600, letterSpacing: '1px', textTransform: 'uppercase', margin: '0 0 2px' }}>{item.label}</p>
                    <p style={{ fontSize: 14, fontWeight: 500, color: '#1A1500', margin: '0 0 2px' }}>{item.value}</p>
                    <p style={{ fontSize: 12, color: '#C4A265', margin: 0 }}>{item.sub}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={{ background: '#1A1500', borderRadius: 16, padding: '22px 20px', marginTop: 20 }}>
              ...
              {[
                ...
                <div key={h.day} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                  ...
                  <span style={{ fontSize: 13, fontWeight: 500, color: '#F5DFA0' }}>{h.hours}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Right — contact form */}
          <div>
            <div style={{ background: '#fff', border: '1px solid #3D2200', borderRadius: 24, padding: isMobile ? '28px 20px' : '36px 32px' }}>
              {formState === 'sent' ? (
                <div style={{ textAlign: 'center', padding: '40px 20px' }}>
                  ...
                  <p style={{ fontSize: 14, color: '#C4A265', lineHeight: 1.65, margin: '0 0 24px' }}>Te responderemos en menos de 24 horas. También puedes contactarnos por WhatsApp para una respuesta más rápida.</p>
                ...
                <>
                  ...
                  <p style={{ fontSize: 13, color: '#C4A265', margin: '0 0 24px' }}>Respondemos en menos de 24 horas</p>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
                    <div>
                      ...
                      <input type="text" placeholder="Tu nombre" value={formData.name} onChange={e => setFormData(d => ({ ...d, name: e.target.value }))}
                        ...
                        onFocus={e => e.target.style.borderColor = '#ffd43a'} onBlur={e => e.target.style.borderColor = errors.name ? '#E24B4A' : '#3D2200'} />
                    ...
                    <div>
                      ...
                      <input type="tel" placeholder="+34 600..." value={formData.phone} onChange={e => setFormData(d => ({ ...d, phone: e.target.value }))}
                        ...
                        onFocus={e => e.target.style.borderColor = '#ffd43a'} onBlur={e => e.target.style.borderColor = '#3D2200'} />
                    </div>
                  </div>
                  ...
                  <div style={{ marginBottom: 14 }}>
                    ...
                    <input type="email" placeholder="tu@email.com" value={formData.email} onChange={e => setFormData(d => ({ ...d, email: e.target.value }))}
                      ...
                      onFocus={e => e.target.style.borderColor = '#ffd43a'} onBlur={e => e.target.style.borderColor = errors.email ? '#E24B4A' : '#3D2200'} />
                  ...
                  <div style={{ marginBottom: 24 }}>
                    ...
                    <textarea placeholder="¿En qué podemos ayudarte?" value={formData.message} onChange={e => setFormData(d => ({ ...d, message: e.target.value }))} rows={5}
                      ...
                      onFocus={e => e.target.style.borderColor = '#ffd43a'} onBlur={e => e.target.style.borderColor = errors.message ? '#E24B4A' : '#3D2200'} />
                    {errors.message && <p style={{ fontSize: 11, color: '#E24B4A', marginTop: 3 }}>{errors.message}</p>}
                  </div>

                  <button onClick={handleSubmit} disabled={formState === 'sending'} style={{ width: '100%', background: formState === 'sending' ? '#C4A265' : '#ffd43a', color: '#fff', border: 'none', borderRadius: 14, padding: '14px', fontFamily: "'Outfit', sans-serif", fontSize: 15, fontWeight: 500, cursor: formState === 'sending' ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                    {formState === 'sending' ? (
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 16 }}>
                    <div style={{ flex: 1, height: 1, background: '#3D2200' }} />
                    <span style={{ fontSize: 12, color: '#C4A265' }}>o</span>
                    <div style={{ flex: 1, height: 1, background: '#3D2200' }} />
                  </div>
      ...
      <section style={{ background: '#1A1500', padding: isMobile ? '48px 20px' : '64px 0' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: isMobile ? '0' : '0 24px', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 32 : 48, alignItems: 'center' }}>
          <div>
            <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: isMobile ? 26 : 34, fontWeight: 600, color: '#F5DFA0', margin: '0 0 16px' }}>Encuéntranos</h2>
