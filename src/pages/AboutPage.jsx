...
export default function AboutPage({ onNavigate }) {
  ...
  const inputStyle = (hasError) => ({
    ...
    border: `1.5px solid ${hasError ? '#E24B4A' : '#3D1800'}`,
    borderRadius: 12, fontFamily: "'Outfit', sans-serif",
    fontSize: 14, color: '#1A0800', background: '#fff',
  ...
  return (
    <div style={{ fontFamily: "'Outfit', sans-serif", background: '#1A0800', minHeight: '100vh' }}>
      ...
      {/* ── HERO ── */}
      <section style={{ background: '#1A0800', padding: isMobile ? '56px 20px 48px' : '80px 0 72px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: isMobile ? '0' : '0 24px', position: 'relative', zIndex: 1 }}>
          ...
          <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: isMobile ? 38 : 58, fontWeight: 600, color: '#FFFFFF', margin: '0 0 20px', lineHeight: 1.0, letterSpacing: '-1px' }}>
            Más de diez años<br />
      ...
      <section style={{ background: '#fff', padding: isMobile ? '48px 20px' : '72px 0' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: isMobile ? '0' : '0 24px', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 32 : 64, alignItems: 'center' }}>
          <div>
            <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: isMobile ? 28 : 38, fontWeight: 600, color: '#1A0800', margin: '0 0 20px', lineHeight: 1.1 }}>
              Empezamos con<br />una brasa y una idea.
            </h2>
            <p style={{ fontSize: 15, color: '#D4956A', lineHeight: 1.8, margin: '0 0 16px' }}>
            ...
            <p style={{ fontSize: 15, color: '#D4956A', lineHeight: 1.8, margin: 0 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            {[
              ...
              <div key={s.label} style={{ background: '#1A0800', border: '1px solid #2A1A00', borderRadius: 16, padding: '22px 18px' }}>
                <p style={{ fontFamily: "'Fraunces', serif", fontSize: 32, fontWeight: 600, color: '#ffd43a', margin: '0 0 4px', lineHeight: 1 }}>{s.num}</p>
                <p style={{ fontSize: 13, fontWeight: 500, color: '#1A0800', margin: '0 0 3px' }}>{s.label}</p>
                <p style={{ fontSize: 11, color: '#D4956A', margin: 0 }}>{s.sub}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── VALUES ── */}
      <section style={{ background: '#1A0800', padding: isMobile ? '48px 20px' : '72px 0' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: isMobile ? '0' : '0 24px' }}>
          <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: isMobile ? 26 : 34, fontWeight: 600, color: '#1A0800', margin: '0 0 8px', textAlign: 'center' }}>Lo que nos define</h2>
          <p style={{ fontSize: 14, color: '#D4956A', textAlign: 'center', margin: '0 0 36px' }}>Por qué nuestros clientes llevan más de diez años volviendo</p>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(4,1fr)', gap: 16 }}>
            {values.map(v => (
              <div key={v.title} style={{ background: '#fff', border: '1px solid #2A1A00', borderRadius: 18, padding: '24px 20px' }}>
                ...
                <h3 style={{ fontFamily: "'Fraunces', serif", fontSize: 17, fontWeight: 600, color: '#1A0800', margin: '0 0 8px' }}>{v.title}</h3>
                <p style={{ fontSize: 13, color: '#D4956A', lineHeight: 1.65, margin: 0 }}>{v.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ background: '#fff', padding: isMobile ? '48px 20px' : '72px 0' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: isMobile ? '0' : '0 24px' }}>
          <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: isMobile ? 26 : 34, fontWeight: 600, color: '#1A0800', margin: '0 0 8px', textAlign: 'center' }}>El equipo</h2>
          <p style={{ fontSize: 14, color: '#D4956A', textAlign: 'center', margin: '0 0 36px' }}>Las personas detrás de cada brasa</p>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3,1fr)', gap: 20 }}>
            {team.map(member => (
              <div key={member.name} style={{ background: '#1A0800', border: '1px solid #2A1A00', borderRadius: 20, padding: '28px 24px', textAlign: 'center' }}>
                <div style={{ width: 72, height: 72, borderRadius: '50%', background: '#1A0800', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32, margin: '0 auto 16px' }}>{member.emoji}</div>
                <h3 style={{ fontFamily: "'Fraunces', serif", fontSize: 19, fontWeight: 600, color: '#1A0800', margin: '0 0 4px' }}>{member.name}</h3>
                <p style={{ fontSize: 13, color: '#ffd43a', fontWeight: 500, margin: '0 0 6px' }}>{member.role}</p>
                <p style={{ fontSize: 12, color: '#D4956A', margin: 0 }}>{member.years}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── CONTACT ── */}
      <section id="contact" style={{ background: '#1A0800', padding: isMobile ? '48px 20px' : '72px 0' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: isMobile ? '0' : '0 24px', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 40 : 64 }}>
          ...
          {/* Left — info */}
          <div>
            <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: isMobile ? 28 : 38, fontWeight: 600, color: '#1A0800', margin: '0 0 12px', lineHeight: 1.1 }}>
              Hablemos
            </h2>
            <p style={{ fontSize: 15, color: '#D4956A', lineHeight: 1.75, margin: '0 0 36px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {[
                ...
                <div key={item.label} style={{ background: '#fff', border: '1px solid #2A1A00', borderRadius: 14, padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 14 }}>
                  ...
                  <div>
                    <p style={{ fontSize: 11, color: '#D4956A', fontWeight: 600, letterSpacing: '1px', textTransform: 'uppercase', margin: '0 0 2px' }}>{item.label}</p>
                    <p style={{ fontSize: 14, fontWeight: 500, color: '#1A0800', margin: '0 0 2px' }}>{item.value}</p>
                    <p style={{ fontSize: 12, color: '#D4956A', margin: 0 }}>{item.sub}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            {/* Hours */}
            <div style={{ background: '#1A0800', borderRadius: 16, padding: '22px 20px', marginTop: 20 }}>
          ...
          {/* Right — contact form */}
          <div>
            <div style={{ background: '#fff', border: '1px solid #2A1A00', borderRadius: 24, padding: isMobile ? '28px 20px' : '36px 32px' }}>
              {formState === 'sent' ? (
                <div style={{ textAlign: 'center', padding: '40px 20px' }}>
                  ...
                  <h3 style={{ fontFamily: "'Fraunces', serif", fontSize: 22, fontWeight: 600, color: '#1A0800', margin: '0 0 10px' }}>¡Mensaje enviado!</h3>
                  <p style={{ fontSize: 14, color: '#D4956A', lineHeight: 1.65, margin: '0 0 24px' }}>Te responderemos en menos de 24 horas. También puedes contactarnos por WhatsApp para una respuesta más rápida.</p>
                ...
                <>
                  <h3 style={{ fontFamily: "'Fraunces', serif", fontSize: 22, fontWeight: 600, color: '#1A0800', margin: '0 0 6px' }}>Envíanos un mensaje</h3>
                  <p style={{ fontSize: 13, color: '#D4956A', margin: '0 0 24px' }}>Respondemos en menos de 24 horas</p>

                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
                    <div>
                      <label style={{ fontSize: 13, fontWeight: 500, color: '#1A0800', display: 'block', marginBottom: 6 }}>Nombre *</label>
                      <input type="text" placeholder="Tu nombre" value={formData.name} onChange={e => setFormData(d => ({ ...d, name: e.target.value }))}
                        style={inputStyle(errors.name)}
                        onFocus={e => e.target.style.borderColor = '#ffd43a'} onBlur={e => e.target.style.borderColor = errors.name ? '#E24B4A' : '#3D1800'} />
                      {errors.name && <p style={{ fontSize: 11, color: '#E24B4A', marginTop: 3 }}>{errors.name}</p>}
                    </div>
                    <div>
                      <label style={{ fontSize: 13, fontWeight: 500, color: '#1A0800', display: 'block', marginBottom: 6 }}>Teléfono</label>
                      <input type="tel" placeholder="+34 600..." value={formData.phone} onChange={e => setFormData(d => ({ ...d, phone: e.target.value }))}
                        style={inputStyle(false)}
                        onFocus={e => e.target.style.borderColor = '#ffd43a'} onBlur={e => e.target.style.borderColor = '#3D1800'} />
                    </div>
                  </div>

                  <div style={{ marginBottom: 14 }}>
                    <label style={{ fontSize: 13, fontWeight: 500, color: '#1A0800', display: 'block', marginBottom: 6 }}>Email *</label>
                    <input type="email" placeholder="tu@email.com" value={formData.email} onChange={e => setFormData(d => ({ ...d, email: e.target.value }))}
                      style={inputStyle(errors.email)}
                      onFocus={e => e.target.style.borderColor = '#ffd43a'} onBlur={e => e.target.style.borderColor = errors.email ? '#E24B4A' : '#3D1800'} />
                    {errors.email && <p style={{ fontSize: 11, color: '#E24B4A', marginTop: 3 }}>{errors.email}</p>}
                  </div>

                  <div style={{ marginBottom: 24 }}>
                    <label style={{ fontSize: 13, fontWeight: 500, color: '#1A0800', display: 'block', marginBottom: 6 }}>Mensaje *</label>
                    <textarea placeholder="¿En qué podemos ayudarte?" value={formData.message} onChange={e => setFormData(d => ({ ...d, message: e.target.value }))} rows={5}
                      ...
                      onFocus={e => e.target.style.borderColor = '#ffd43a'} onBlur={e => e.target.style.borderColor = errors.message ? '#E24B4A' : '#3D1800'} />
                    {errors.message && <p style={{ fontSize: 11, color: '#E24B4A', marginTop: 3 }}>{errors.message}</p>}
                  </div>

                  <button onClick={handleSubmit} disabled={formState === 'sending'} style={{ width: '100%', background: formState === 'sending' ? '#D4956A' : '#ffd43a', color: '#fff', border: 'none', borderRadius: 14, padding: '14px', fontFamily: "'Outfit', sans-serif", fontSize: 15, fontWeight: 500, cursor: formState === 'sending' ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                    {formState === 'sending' ? (
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 16 }}>
                    <div style={{ flex: 1, height: 1, background: '#3D1800' }} />
                    <span style={{ fontSize: 12, color: '#D4956A' }}>o</span>
                    <div style={{ flex: 1, height: 1, background: '#3D1800' }} />
                  </div>
      ...
      {/* ── MAP PLACEHOLDER ── */}
      <section style={{ background: '#1A0800', padding: isMobile ? '48px 20px' : '64px 0' }}>
