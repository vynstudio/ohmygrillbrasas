  ...
  return (
    <div
      ...
      style={{
        background: '#fff',
        borderRadius: 24,
        border: `2px solid ${hovered ? accent : '#3D1800'}`,
        overflow: 'hidden',
    ...
    >
      {/* Top band */}
      <div style={{ background: '#1A0800', padding: '28px 28px 24px', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '20px 28px', flex: 1 }}>
        <p style={{ fontSize: 11, letterSpacing: '2px', color: '#D4956A', fontWeight: 600, textTransform: 'uppercase', marginBottom: 12 }}>Incluye</p>
        <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
          {pack.items.map((item, i) => (
            <li key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 0', borderBottom: i < pack.items.length - 1 ? '1px solid #1A1000' : 'none' }}>
              ...
              <span style={{ fontSize: 14, color: '#1A0800' }}>{item}</span>
            </li>
          ))}
        </ul>
      </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 10 }}>
              <span style={{ fontFamily: "'Fraunces', serif", fontSize: 34, fontWeight: 600, color: '#1A0800', lineHeight: 1 }}>€{pack.price}</span>
              <span style={{ fontSize: 14, color: '#D4956A', textDecoration: 'line-through' }}>€{pack.originalPrice}</span>
            </div>
          ...
          <div style={{ textAlign: 'right' }}>
            <p style={{ fontSize: 11, color: '#D4956A', margin: '0 0 2px' }}>Precio por persona</p>
            <p style={{ fontFamily: "'Fraunces', serif", fontSize: 16, fontWeight: 600, color: '#D4956A', margin: 0 }}>
...
function ComparisonTable({ isMobile }) {
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 20, border: '1px solid #2A1A00', overflow: 'hidden' }}>
      {/* Header */}
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '120px repeat(3,1fr)' : '180px repeat(3,1fr)', background: '#1A0800' }}>
      ...
      {/* Rows */}
      {rows.map((row, ri) => (
        <div key={row.label} style={{ display: 'grid', gridTemplateColumns: isMobile ? '120px repeat(3,1fr)' : '180px repeat(3,1fr)', background: ri % 2 === 0 ? '#fff' : '#1A0800', borderTop: '1px solid #F0EDE8' }}>
          <div style={{ padding: isMobile ? '12px 16px' : '14px 20px', display: 'flex', alignItems: 'center' }}>
            <span style={{ fontSize: isMobile ? 12 : 13, fontWeight: 500, color: '#D4956A' }}>{row.label}</span>
          </div>
          {row.vals.map((val, vi) => (
            <div key={vi} style={{ padding: isMobile ? '12px 8px' : '14px 12px', textAlign: 'center', borderLeft: '1px solid #F0EDE8', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span style={{ fontSize: isMobile ? 12 : 13, color: '#1A0800', fontWeight: row.label === 'Ahorro' ? 600 : 400, color: row.label === 'Ahorro' ? accents[vi] : '#1A0800' }}>{val}</span>
            </div>
          ))}
        </div>
      ))}
    ...
    <div style={{ fontFamily: "'Outfit', sans-serif", background: '#1A0800', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: isMobile ? '32px 20px' : '48px 24px' }}>
        ...
        <div style={{ marginBottom: 56 }}>
          <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: isMobile ? 24 : 30, fontWeight: 600, color: '#1A0800', margin: '0 0 6px' }}>Compara los packs</h2>
          <p style={{ fontSize: 14, color: '#D4956A', margin: '0 0 24px' }}>¿No sabes cuál elegir? Aquí lo ves claro.</p>
        ...
        {/* FAQ */}
        <div style={{ marginBottom: 56 }}>
          <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: isMobile ? 24 : 30, fontWeight: 600, color: '#1A0800', margin: '0 0 20px' }}>Preguntas frecuentes</h2>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 14 }}>
            {[
              ...
              <div key={faq.q} style={{ background: '#fff', border: '1px solid #2A1A00', borderRadius: 14, padding: '20px 22px' }}>
                <h4 style={{ fontFamily: "'Fraunces', serif", fontSize: 16, fontWeight: 600, color: '#1A0800', margin: '0 0 8px' }}>{faq.q}</h4>
                <p style={{ fontSize: 13, color: '#D4956A', lineHeight: 1.65, margin: 0 }}>{faq.a}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* CTA */}
        <div style={{ background: '#1A0800', borderRadius: 24, padding: isMobile ? '36px 24px' : '48px 56px', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr auto', gap: 24, alignItems: 'center' }}>
          <div>
